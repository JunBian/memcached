...
static void settings_init(void) {
    ...
    settings.port = 11211;// default port the pro. use.
    settings.udpport = 11211;
    /* By default this string should be NULL for getaddrinfo() */
    settings.inter = NULL;// this is may be the ip address.
    settings.maxbytes = 64 * 1024 * 1024; /* default is 64MB. the memory that  the pro. uses  */
    settings.maxconns = 1024;         /* 最大链接数 to limit connections-related memory to about 5MB */
    settings.verbose = 0;
    settings.oldest_live = 0;		// 最长的存放时间，最大是永不过期
    ...
    settings.binding_protocol = negotiating_prot;/*发现协议*/
...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    if (NULL == c) {//如果为空的话，再进行创建,某个客户端第一次创建链接
 ...
 * happen here.
 * 在状态机中，设置链接的当前的状态。任何特殊的需要在某个状态转换发生
 * 的处理过程可以在这里设定
 */
static void conn_set_state(conn *c, enum conn_states state) {
    assert(c != NULL);
...
/*
 * 从网络中尽量地读取数据，处理溢出和链接关闭，
 * 在读之前，把剩余的一个命令的未完全的片移动到buffer的开始位置。
 *
 * 为了保护我们避免其他人泛洪链导致链接吃掉所有的可以使用的内存，
 * 
...
static enum try_read_result try_read_network(conn *c) {
    ...
    while (1) {
        ...
        int avail = c->rsize - c->rbytes;/* 剩余的内存空间，可以用来存储数据的内存大小*/
        res = read(c->sfd, c->rbuf + c->rbytes, avail);/* 从sfd描述的文件(也就是socket)中
							读取avail的内容到 *(rbuf+rbytes)
							的缓存中
							要注意的是，每次读的话，是偏移rbytes
							个内存位置*/
        if (res > 0) {//读取到内容
            pthread_mutex_lock(&c->thread->stats.mutex);
            c->thread->stats.bytes_read += res;// 这个为什么要进行互斥处理？
            pthread_mutex_unlock(&c->thread->stats.mutex);
            gotdata = READ_DATA_RECEIVED;
            c->rbytes += res;//注意的是，这个要进行偏移，因为read()的时候要使用 c-bur + c ->rbytes
            if (res == avail) {//刚好读满
                continue;
            } else {
                break;
...
void do_accept_new_conns(const bool do_accept) {
    ...
    for (next = listen_conn; next; next = next->next) {
        if (do_accept) {
            update_event(next, EV_READ | EV_PERSIST);// EV_READ|EV_PRESIST = 10010 
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
        ...
        switch(c->state) {
        case conn_listening://监听
            addrlen = sizeof(addr);//地址的长度
            if ((sfd = accept(c->sfd, (struct sockaddr *)&addr, &addrlen)) == -1) {//socket监听
                if (errno == EAGAIN || errno == EWOULDBLOCK) {// errno 是线程的一个状态值
                    /* these are transient, so don't log anything */
                    stop = true;
                } else if (errno == EMFILE) {
                    if (settings.verbose > 0)
                        fprintf(stderr, "Too many open connections\n");
                    accept_new_conns(false);//设定不在接受新的链接
                    stop = true;
                } else {
                    perror("accept()");
                    stop = true;
            ...
            dispatch_conn_new(sfd, conn_new_cmd, EV_READ | EV_PERSIST,
                                     DATA_BUFFER_SIZE, tcp_transport);// 开启新的链接
            stop = true;
            break;
...
/**
 * Do basic sanity(健全的) check of the runtime environment
...
static bool sanitycheck(void) {
    ...
    const char *ever = event_get_version();//查看版本，暂时不明白其原理，从代码来看，应该是获memcached的版本
    if (ever != NULL) {
...
int main (int argc, char **argv) {
    ...
    if (!sanitycheck()) {//检测系统的状态，其实就是检查memcached的版本是否在1.3以上
        return EX_OSERR;
    }
    ...
    while (-1 != (c = getopt(argc, argv,
		...
		/*"optarg"指的便是获取到的参数的内容，详细可见 getopt的声明*/
        switch (c) {
        case 'a':
        ...
        case 'f':
            ...
            if (settings.factor <= 1.0) {// 增长因此只能是大于1的数
                fprintf(stderr, "Factor must be greater than 1\n");
                return 1;
            }
            break;
        case 'n': 
	    /*每个块的最小分配空间  key + value + flag*/
    ...
    // 其中，maxbytes应该指的是总的内存大小空间
    //preallocate 如若是true表示提前分配内存空间，默认是false
    // 如果指定了"L"参数，那么就是为true，为了能够工作的时候快
    // 点，可以在启动的时候配置该参数:
