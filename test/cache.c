...
/**
 * 分配一个内存，它的大小是 cache_t(其实存放的就是cache_t类型数据)
 * 并用 name、constructor,destrutor进行初始化、同时，设定freetotal
 * 为 64 ，bufize 未bufsize 加上 两倍的redzone_pattern的大小
 * 这个方法本身是线程安全的，不用在做任何的同步
 *
 */
cache_t* cache_create(const char *name, size_t bufsize, size_t align,
    ...
    char* nm = strdup(name);/* create a new String which is the same as "name"
    				也就是复制、拷贝 */
    void** ptr = calloc(initial_pool_size, sizeof(void*));
    printf("initial pool :%i\n",initial_pool_size);
    printf("void* size :%i\n",sizeof(void*));
