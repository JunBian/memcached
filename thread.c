...
struct conn_queue_item {
    int               sfd;//链接描述
    enum conn_states  init_state;//链接状态
    int               event_flags;//时间标记
    int               read_buffer_size;//读的buffer大小
    enum network_transport     transport;//传输协议，是本地的，tcp的还是udp协议。
    CQ_ITEM          *next;//队列的下一项
};
...
struct conn_queue {
    CQ_ITEM *head;//队列头
    CQ_ITEM *tail;//队列尾巴
    pthread_mutex_t lock;//队列的锁，估计是用于取项的时候的安全锁。
...
static void create_worker(void *(*func)(void *), void *arg) {
    ...
    if ((ret = pthread_create(&thread, &attr, func, arg)) != 0) {//arg 是线程的参数。
...
static void setup_thread(LIBEVENT_THREAD *me) {
   ...
   /*
    * 设置监听的句柄函数，当有命令来的时候，threaad_libevent_process调用
    * */
...
static void thread_libevent_process(int fd, short which, void *arg) {
    ...
    item = cq_pop(me->new_conn_queue);/*弹出一个可用的元素*/
 ...
 * 分配一个新的线程给一个新的线程。这个方法只有在主线程调用，或者在初始化的
 * 时候，或者因为一个接入链接的时候。
 */
